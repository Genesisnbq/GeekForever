package _02_Java_Object_Oriented_Programming._38_Overlay.supermarket;
...
public class Phone extends Merchandise {
    ...
    //>> TODO ↓↓↓↓↓↓↓↓↓下面才是继承的终极奥义↓↓↓↓↓↓↓↓↓↓↓↓
    //>> TODO 也就是说, 子类并不是只把父类的方法拿过来, 而且可以通过覆盖来替换其中不适合子类的方法
    //>> TODO ↑↑↑↑↑↑↑↑↑上面才是继承的终极奥义↑↑↑↑↑↑↑↑↑↑↑↑
    //>> TODO 题外话: 属性是联动的, 可能是特殊意义的。
    // TODO 所以直接给属性赋值时危险的, 因为没有办法检查新的值是否有意义, 也没法对这个修改做联动的修改

    public double buy(int count) {
        //TODO 这个方法代码大部分和父类一样, 坑定有方法解决
        if (count > MAX_BUY_ONE_ORDER) {
            System.out.println("购买失败, 手机一次最多可以买" + MAX_BUY_ONE_ORDER + "个");
            return -2;
        }
        if (this.count < count) {
...
//    }
    ...
    // TODO 即使在父类里, 只是一个简单的getName, 但是这样做, 子类就饿可以覆盖掉父类的方法
    ...
    //TODO 同样的, 我们想要调用父类里的describe方法, 试试?
